        ...
        while salida:
            ...
            if (existe_o_no):
                ...
                if (cuentas[numero][usuario]['password']==password):
                    ...
                    cargar_servicios(usuario,temp_p[0])
...
def editar_registros(mega_dic,cuentas,usuario,indice):
    ...
    if (opcion=='2'):
        ...
        while True:
            ...
            if len(nueva)==0:
                system('clear')
                print('Necesitas al menos 3 caracteres')
            if ((len(nueva)>=3 )and (len(nueva)!=0)):
                break
        temp_p[0]=nueva      
        #print(temp_p[0],nueva)
    ...
    if(opcion=='3'):
        ...
        if(len(cuentas[indice][usuario]['servicios'])!=0):
            ...
            if (el=='1' or el=='4'):
                ...
                usuario_old=str(encript(temp_p[0],usuario_old))
            if(el=='2'or el=='4'):
                password_old=input('Ingresa la nueva contraseña: ')
                password_old=str(encript(temp_p[0],password_old))
            if(el=='3' or el=='4'):
                NombreServicio_old=input('Ingresa el nombre del servicio: ')
                NombreServicio_old=str(encript(temp_p[0],NombreServicio_old))
            '''
...
def opciones_del_menu(nombre_usuario,op,mega_dic,cuentas,indice_aux):
    ...
    if(op=='1'):
        ...
        #print(temp_p[0])
        nombre_servicio=str(encript(temp_p[0],str(nombre_servicio)))
        password=str(encript(temp_p[0],str(password)))
        nuevo_usuario=str(encript(temp_p[0],str(nuevo_usuario)))
        '''
        ...
        cargar_servicios(nombre_usuario,temp_p[0])
        '''
    ...
    if(op=='2'):
        ...
        if(len(cuentas[indice_aux][nombre_usuario]['servicios'])!=0):
            ...
            validos=['1','2']
            while True:
                borrar=input("[1] SI [2]| NO >> ")
                if borrar in validos:
                    break
                else:
                    print('Solo opciones validas')
...
try:
    x,y=doc.abrir_documento()

    if(x==True and len(y)==2):
        estado,user_name=login_2('2')
    ...
    print(e)
    print('Reinicia el programa >:C :/ °o° ')
